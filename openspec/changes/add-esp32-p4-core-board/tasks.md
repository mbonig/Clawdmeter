...
- [x] 8.3 End-to-end verified: advertises as "Clawdmeter", discoverable via `bleak`, and paired successfully through real macOS Bluetooth settings. **Not yet verified**: actual usage-JSON write from the real host daemon, the REQ/notify refresh-request flow, and HID Space keypress reaching the OS — these need a live daemon pairing session to confirm, unlike the GATT/pairing layer already confirmed here.
