...
- [x] 1.6 Write up spike findings and update design.md's Open Questions — done (see design.md's "Spike Findings — Round 2"). **Superseded by a follow-up session (2026-07-24): BLE is portable after all, via a path this spike never considered — arduino-esp32's own bundled `BLE` library (not `h2zero/NimBLE-Arduino`, not the ESP-IDF-native `esp-nimble-cpp` stack). See design.md's "BLE resolution" section and section 8 below.**
...
- [x] 8.1 **Resolved in a follow-up session (2026-07-24) — BLE works.** `h2zero/NimBLE-Arduino` still fails to build for ESP32-P4 exactly as the spike found, but arduino-esp32 core's own *bundled* `BLE` library (`#include <BLEDevice.h>`, a different, older-generation API — not part of the NimBLE-Arduino family at all) links cleanly against the framework's already-precompiled NimBLE-over-ESP-Hosted stack (`CONFIG_ESP_HOSTED_ENABLE_BT_NIMBLE=1` + `libbt.a`, both already present in the pinned `framework-arduinoespressif32-libs` package). No SDIO pin config needed — arduino-esp32's generic `esp32p4` variant defaults already match Waveshare's own schematic. Confirmed via `esptool`/real hardware throughout, plus a real BLE scan and a genuine pairing through the user's own macOS Bluetooth settings.
- [x] 8.2 Shared `ble.cpp`, `main.cpp`, and every other file remain untouched — this board's env excludes `ble.cpp` from `build_src_filter` and substitutes `boards/waveshare_p4_touch_lcd_5/ble.cpp`, a full reimplementation of the same `ble.h` contract (custom data service, single-owner bond lock via raw `ble_store_util_*` calls, HID keyboard, connection-parameter pushback) against the bundled library's different API. See that file's header comment for the complete API-difference list and design.md's "BLE resolution" section for the full story, including a real crash (`BLEHIDDevice::manufacturer(String)` null-pointer bug) found and fixed along the way.
- [x] 8.3 End-to-end verified: advertises as "Clawdmeter", discoverable via `bleak`, paired successfully through real macOS Bluetooth settings, and the real host daemon delivered live usage JSON to the board (confirmed in daemon logs). Hit and resolved a real post-pairing bonding desync along the way — see design.md's "Post-pairing bonding desync" section — fixed by a GUI forget + re-pair, plus a new `clearbonds` serial command (this board has no PWR button to trigger the existing hold-to-pair gesture). **Not yet verified**: the REQ/notify refresh-request flow and HID Space keypress reaching the OS as an actual keystroke.
