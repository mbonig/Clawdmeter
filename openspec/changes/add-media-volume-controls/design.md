...
- **[Risk, discovered during implementation — now RESOLVED]** This feature was the first to expose apparent GT911 coordinate-reliability problems (taps landing 150-300px short near the bottom, and genuinely non-monotonic X readings near the horizontal centre). The multi-touch point-slot hypothesis recorded here originally was **wrong**, as were the mitigations built on it. Actual cause: `touch.cpp` read the point record from `0x8150` assuming that byte was the track id (the record starts at `0x814F`) and treated the little-endian coordinate pairs as big-endian, so it combined bytes from adjacent fields — `x = (X_high << 8) | Y_low`, `y = (Y_high << 8) | size_low`. Fixed by matching Espressif's official `esp_lcd_touch_gt911` layout; all the empirical mitigations (sample-agreement filter, point-count filter, 5/4 X scale, 300px dead zone, 100px bezel inset) were removed as artifacts. See tasks.md section 9.
- **[Risk, discovered during implementation — now RESOLVED]** Two further blockers had to be fixed before the feature could work at all, neither anticipated by this design: (1) arduino-esp32's bundled BLE library registers only one characteristic per UUID per service, so the second `inputReport()` (both use `0x2A4D`) was never added to the GATT database — fixed by vendoring the library to `firmware/lib/BLE/` and patching `addCharacteristic()`; (2) macOS requires LE Secure Connections *and* an OS-flow pairing before it will attach HID and subscribe to report CCCDs. See CLAUDE.md's P4 section.
...
- ~~Whether the touch-coordinate issue is really GT911 point-slot confusion or something else~~ — **answered: neither.** It was an off-by-one register offset plus a byte-order error in this board's own `touch.cpp`. See Risks above.
