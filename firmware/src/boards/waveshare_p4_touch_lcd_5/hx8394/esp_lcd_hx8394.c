// Vendored (MIT license, see LICENSE in this folder) from Waveshare's
// ESP32-P4-WIFI6-Touch-LCD-5 example repo, not pulled via PlatformIO's
// component manager: `framework = arduino` never invokes it (idf_component.yml
// is silently ignored under Arduino), confirmed empirically before choosing
// this approach. See openspec/changes/add-esp32-p4-core-board/design.md.
...
// #include "i2c_bus.h" // unused, only referenced in commented-out example code below

// Normally auto-generated by the ESP-IDF component manager from
// idf_component.yml's declared version — not available since this file is
// vendored directly rather than pulled as a managed component. Cosmetic only
// (used solely in the boot log line below).
