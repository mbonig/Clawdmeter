...
void BLEService::addCharacteristic(BLECharacteristic *pCharacteristic) {
  ...
  // ---------------------------------------------------------------------------
  // CLAWDMETER PATCH (see PROGRESS.md / boards/waveshare_p4_touch_lcd_5/ble.cpp)
  //
  // Upstream, the NimBLE branch here was `if (pExisting != nullptr) {
  // pExisting->m_removed = 0; } else { setByUUID(...); }` — so whenever a
  // characteristic was added whose UUID was already present, the NEW object was
  // silently never inserted into m_characteristicMap. It just cleared m_removed on
  // the pre-existing one and returned. The new BLECharacteristic stayed a live C++
  // object that was never registered with the GATT stack, so its handle remained
  // 0xFFFF (NULL_HANDLE) forever and it was invisible to any host.
  //
  // That breaks HID devices with more than one input report: every HID report
  // characteristic uses UUID 0x2A4D, so only the first BLEHIDDevice::inputReport()
  // call ever registered. Proven on hardware with an order-swap experiment —
  // whichever report was created *second* came back with handle 0xFFFF. Duplicate
  // UUIDs are legal in GATT (reports are disambiguated by their 0x2908 Report
  // Reference descriptor), and m_characteristicMap is keyed by pointer, not UUID,
  // so it holds duplicates just fine.
  //
  // Fixed by only taking the un-remove path when the *same object* is being
  // re-added (the dynamic-service re-add case that branch was written for), and
  // otherwise registering the new characteristic normally.
  // ---------------------------------------------------------------------------
#if defined(CONFIG_NIMBLE_ENABLED)
  if (pExisting == pCharacteristic) {
