...
- Touch: **GT911** via I2C (SDA=7, SCL=8, addr=0x5D, no RST/INT pins wired on this board — hand-rolled inline reader in `touch.cpp`, no `esp_lcd_touch` dependency). **This specific unit's coordinate bytes are high-byte-first** (`x = (rec[1]<<8) | rec[2]`), the opposite order from some published GT911 register maps — found by dumping raw register bytes against a known tap location; don't trust the datasheet's byte order blindly on a new GT911 unit.
  - **Known issue, not fully resolved (found 2026-07-24 while adding the media-controls control bar):** raw touch coordinates are unreliable enough to make small, closely-spaced touch targets risky. A top-of-screen calibration tap lands pixel-perfect, and the controller's own configured resolution registers (0x8146/0x8148) read exactly 720x1280 — ruling out a scale/axis-swap bug — but deliberate near-bottom taps have landed 150-300+ px short on different occasions, and one test found a genuinely non-monotonic result (tapping further right produced a *lower* X reading) near the horizontal center. Most likely GT911 multi-touch point-slot confusion — the driver always reads point 1's record at 0x8150 with no way to detect if a given sample actually belongs to a different slot. `touch_hal_read()` now takes two independent samples per call and rejects results disagreeing by more than 40px (filters single-sample torn/corrupt reads) but this does **not** fully fix a consistently-wrong-but-self-agreeing read. Don't place small, closely-spaced touch targets without generous margins on this board until this is properly root-caused (disambiguating all 5 GT911 point slots, or diffing against Waveshare's reference driver) — see `openspec/changes/add-media-volume-controls/tasks.md` section 9 for the tracked follow-up.
