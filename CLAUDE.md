...
- `boards/waveshare_p4_touch_lcd_5/` — Waveshare ESP32-P4-WIFI6-Touch-LCD-5 (HX8394 **MIPI-DSI** panel, 720×1280 portrait, GT911 touch, ESP32-P4 SoC). Build env: `waveshare_p4_touch_lcd_5`. **First MIPI-DSI port and first RISC-V-P4 port; verified on physical hardware (2026-07-23). BLE is not wired up** — see the dedicated section below.
...
- Buttons: GPIO 0 (BOOT → Space/voice-mode) only. **No PWR button** — this board's only other control is a hardware RST line, not a readable GPIO (same posture as `waveshare_lcd_185b`), so splash-cycling / hold-to-pair are unavailable.
...
- **BLE is not wired up.** `h2zero/NimBLE-Arduino` fails to build for ESP32-P4 outright — confirmed on real hardware (a genuine compiler error in its own FreeRTOS porting layer, not a config issue) and corroborated by the library's own maintainer (h2zero/NimBLE-Arduino#906: *"This repo cannot support [ESP32-P4]... You will need to use esp-nimble-cpp and esp-idf"*). The ESP-IDF-native fallback (`esp-nimble-cpp` + `esp_hosted` + `esp_wifi_remote`) also fails today under PlatformIO in both `framework = arduino` and `framework = espidf` (independently reported: h2zero/esp-nimble-cpp#377), traced here to the same `idf_component.yml`-is-ignored issue — `esp-nimble-cpp`'s ESP32-P4 support depends on transitively pulling `esp_hosted`/`esp_wifi_remote` as managed components declared in *its own* `idf_component.yml`, which never fires. This board's `[env:waveshare_p4_touch_lcd_5]` excludes shared `ble.cpp` from `build_src_filter` and substitutes `ble_stub.cpp` (implements the same `ble.h` contract as a permanent no-op) — zero changes to any other shared file. Onboard co-processor for when this is revisited: **ESP32-C6-MINI** over SDIO (ESP-Hosted).
- Flash: 32 MB. Uses `default_32MB.csv` partition table.
